...
public class Missile {
    ...
    public int tankId ;                               //记录子弹是哪个坦克发出来的
    Direction dir ;

    Missile(int x, int y, Direction dir,int groupId,TankInterface tankInterface,int tankId) {
        this.x = x;
        this.y = y;
        this.dir = dir;
    ...
    public void hitTank(EnemyTank tank){
        if(this.getRectangle().intersects(tank.getRectangle()) && this.groupId != tank.groupId){
            this.live = false ;
            tank.live =false ;
            tankInterface.tanks.remove(tank);                      //坦克被打到移除容器中的该坦克
